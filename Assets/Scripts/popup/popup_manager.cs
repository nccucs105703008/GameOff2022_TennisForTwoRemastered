...
public class popup_manager : MonoBehaviour
{
	...
	public GameObject itemSelector_window;
	public GameObject charaSelector_window;
    ...
    {
		if (popLayer == null) {
			instance.popLayer = Instantiate(FileManager.LoadPrefab("popup/pop_layer"));
			DontDestroyOnLoad(instance.popLayer);
		}
    ...
    public void show_normal_window(string title, string content, Action yesCB, Action noCB,bool needSE_fromCloseManager = true, Action openCB = null, Action closeCB = null, bool onlyOpenCB = false, bool onlyCloseCB = false, pop_item.popType popType = pop_item.popType.normal, string btnLabel1 = "", string btnLabel2 = "")
    {
        ...
        {
            GameObject temp = Instantiate(FileManager.LoadPrefab("popup/popup_normal"), Get_popLayer());
        ...
        {
            GameObject temp_popup_normal_window = Instantiate(FileManager.LoadPrefab("popup/popup_normal"), Get_popLayer());
    ...
    public void show_setting_window(bool needSE_fromCloseManager = true, Action openCB = null, Action closeCB = null, bool onlyOpenCB = false, bool onlyCloseCB = false, string btnLabel1 = "", string btnLabel2 = "")
    {
        ...
        {
            setting_window = Instantiate(FileManager.LoadPrefab("setting_window"), Get_popLayer());
        }
        else
        {
            int count = popup_manager.GetInstance().Get_popLayer().childCount;
    ...
    public void show_gameMenu(bool needSE_fromCloseManager = true, Action openCB = null, Action closeCB = null, bool onlyOpenCB = false, bool onlyCloseCB = false)
    {
        ...
        {
            poppup_gameMenu = Instantiate(FileManager.LoadPrefab("popup/poppup_gameMenu"), Get_popLayer());
        }
        else
        {
            int count = popup_manager.GetInstance().Get_popLayer().childCount;
	...
	public void show_save_load_window(int type) //0 load 1 save 2 new game (save and start)
{
		if (save_load_window == null) {
			save_load_window = Instantiate(FileManager.LoadPrefab("save_load_window"), Get_popLayer());
		}
		else {
			int count = popup_manager.GetInstance().Get_popLayer().childCount;
    ...
    {
		for (int i = popup_normal_window.Count - 1; i >= 0; i--) {
			if (popup_normal_window[i] != null) {
				popup_normal_window.RemoveAt(i);
			}
		}
		...
		if (save_load_window != null) save_load_window.SetActive(false);
		if (setting_window != null) setting_window.SetActive(false);
		if (itemSelector_window != null) itemSelector_window.SetActive(false);
		if (charaSelector_window != null) charaSelector_window.SetActive(false);

		for (int i = Get_popLayer().childCount - 1; i >= 0; i--) {
			if (Get_popLayer().GetChild(i) != null) {
				Get_popLayer().GetChild(i).gameObject.SetActive(false);
			}
		}
	}
}
